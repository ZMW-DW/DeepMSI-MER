...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        dialogue_id = self.data.iloc[idx]['Dialogue_ID']
        utterance_id = self.data.iloc[idx]['Utterance_ID']
        file_name = f"dia{dialogue_id}_utt{utterance_id}"
...
def main(local_rank, args):
    ...
    # 创建dataset以及合并
    train_dataset = MyDataset(file=CFG.MELD_train, video_dir=CFG.MELD_train_video_path, text_dir=CFG.MELD_train_text_path, audio_dir=CFG.MELD_train_audio_path)
    valid_dataset = MyDataset(file=CFG.MELD_valid, video_dir=CFG.MELD_valid_video_path, text_dir=CFG.MELD_valid_text_path, audio_dir=CFG.MELD_valid_audio_path)
    test_dataset = MyDataset(file=CFG.MELD_test, video_dir=CFG.MELD_test_video_path, text_dir=CFG.MELD_test_text_path, audio_dir=CFG.MELD_test_audio_path)
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(full_dataset)):
        ...
        if local_rank == 0 and best_label_true is not None and best_label_pred is not None:
            ...
            # 使用 numpy 保存
            np.save(f'../results/MELD/fold_{fold}_best_labels_by_class.npy', fold_best_labels)
            # 或者使用 pickle 保存
